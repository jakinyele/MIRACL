...
public:
    ...
    int to_binary(const Big&,int,char *,BOOL justify=FALSE);
    ...
    Big luc(const Big& ,const Big&, const Big&, Big *b4=NULL);
...
// This next only works if MIRACL is using a binary base...
    ...
    int window(const Big&,int,int*,int*,int window_size=5);
    friend int window(const Big&,int,int*,int*,int window_size);
    int naf_window(const Big&,const Big&,int,int*,int*,int store=11);
